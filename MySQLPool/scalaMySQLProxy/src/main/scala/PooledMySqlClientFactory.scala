...
object CreateMySqlPool {
  ...
  def apply(): GenericObjectPool[MySqlProxy] = {
    if (this.genericObjectPool == null) {
      this.synchronized {
        val jdbcUrl = ConfigurationManager.config.getString(Constant.JDBC_URL)
        val jdbcUser = ConfigurationManager.config.getString(Constant.JDBC_USER)
        val jdbcPassword = ConfigurationManager.config.getString(Constant.JDBC_PASSWORD)
        val size = ConfigurationManager.config.getInt(Constant.JDBC_DATASOURCE_SIZE)
